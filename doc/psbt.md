# PSBT Howto for PuertoRicoTainoCoin Core

Since PuertoRicoTainoCoin Core 0.17, an RPC interface exists for Partially Signed PuertoRicoTainoCoin
...
PSBT is an interchange format for PuertoRicoTainoCoin transactions that are not fully signed
...
Overall, the construction of a fully signed PuertoRicoTainoCoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid PuertoRicoTainoCoin transaction (in network format)
...
## PSBT in PuertoRicoTainoCoin Core
...
#### Multisig with multiple PuertoRicoTainoCoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
PuertoRicoTainoCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `puertoricotainocoin-cli` is used.
